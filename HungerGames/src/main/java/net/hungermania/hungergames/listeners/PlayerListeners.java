...
public class PlayerListeners extends GameListener {
    ...
    public void onPlayerChat(AsyncPlayerChatEvent e) {
        if (e.getMessage().startsWith("@") || e.getMessage().startsWith("$")) { return;}
        Game game = plugin.getGameManager().getCurrentGame();
        if (game == null) {
            User user = ManiaCore.getInstance().getUserManager().getUser(e.getPlayer().getUniqueId());
        ...
        format = format.replace("{message}", e.getMessage());
        if (StringUtils.isNotEmpty(rank.getChatColor())) {
            if (rank.getChatColor() == null) {
                format = format.replace("{chatcolor}", "&f");
            }
            format = format.replace("{chatcolor}", rank.getChatColor());
        ...
        if (game.getState() != State.ENDING) {
            if (game.getSpectatorsTeam().isMember(user.getUniqueId()) || game.getHiddenStaffTeam().isMember(user.getUniqueId())) {
                e.setCancelled(true);
                Set<UUID> watching = new HashSet<>(game.getSpectatorsTeam().getMembers());
                watching.addAll(game.getHiddenStaffTeam().getMembers());
                for (UUID s : watching) {
                ...
                new BukkitRunnable() {
                    public void run() {
                        ...
                        if (game != null) {
                            ...
                            user.getBukkitPlayer().teleport(Bukkit.getWorld("world").getSpawnLocation());
