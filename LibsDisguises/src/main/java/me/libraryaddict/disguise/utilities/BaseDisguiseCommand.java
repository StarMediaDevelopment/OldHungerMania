...
public abstract class BaseDisguiseCommand implements CommandExecutor {
    ...
    protected Disguise parseDisguise(CommandSender sender, String[] args, HashMap<DisguiseType, HashMap<ArrayList<String>, Boolean>> map) throws DisguiseParseException, IllegalAccessException, InvocationTargetException {
        ...
        if (args[0].startsWith("@")) {
            ...
            optionPermissions = (map.containsKey(disguise.getType()) ? map.get(disguise.getType()) : new HashMap<ArrayList<String>, Boolean>());
        } else {
            ...
            if (disguiseType.isPlayer()) {
                ...
                if (args.length == 1) {
                    ...
                    if (!disguiseOptions.isEmpty() && (!disguiseOptions.containsKey(args[1].toLowerCase()) || !disguiseOptions.get(args[1].toLowerCase()))) {
                        ...
                        if (int.class == param) {
                            ...
                            if (split.length == 4) {
                                for (int a = 0; a < 4; a++) {
                                    try {
                                        ...
                                        throw parseToException("item ID,ID,ID,ID" + ChatColor.RED + " or " + ChatColor.GREEN + "ID:Data,ID:Data,ID:Data,ID:Data combo", valueString, methodName);
                                    }
                                }
                            } else {
                                throw parseToException("item ID,ID,ID,ID" + ChatColor.RED + " or " + ChatColor.GREEN + "ID:Data,ID:Data,ID:Data,ID:Data combo", valueString, methodName);
                            ...
                            try {
                                for (RabbitType type : RabbitType.values()) {
                                    if (type.name().replace("_", "").equalsIgnoreCase(valueString.replace("_", "").replace(" ", ""))) {
                                        value = type;
                                        break;
                                    }
                                }
    ...
    private Entry<Method, Integer> getMethod(Method[] methods, String methodName, int toStart) {
        for (int i = toStart; i < methods.length; i++) {
            ...
            if (!method.getName().startsWith("get") && method.getName().equalsIgnoreCase(methodName) && method.getAnnotation(Deprecated.class) == null && method.getParameterTypes().length == 1) {
