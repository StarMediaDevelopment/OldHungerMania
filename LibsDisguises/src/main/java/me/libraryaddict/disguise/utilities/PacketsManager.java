...
public class PacketsManager {
        ...
        if (clientInteractEntityListener != null) {
            ...
            mainListener = new PacketAdapter(libsDisguises, ListenerPriority.HIGH, packetsToListen) {
                ...
                public void onPacketSending(PacketEvent event) {
                    ...
                    StructureModifier<Entity> entityModifer = event.getPacket().getEntityModifier(observer.getWorld());
                    org.bukkit.entity.Entity entity = entityModifer.read((PacketType.Play.Server.COLLECT == event.getPacketType() ? 1 : 0));
            ...
            if (disguise != null) {
                ...
                // Else if the packet is sending entity metadata
                    ...
                    if (DisguiseConfig.isMetadataPacketsEnabled()) {
                        List<WrappedWatchableObject> watchableObjects = disguise.getWatcher().convert(packets[0].getWatchableCollectionModifier().read(0));
                ...
                // Else if the disguise is attempting to send players a forbidden packet
                    ...
                    if (disguise.getType().isMisc() || (packets[0].getIntegers().read(1) == 2 && (!disguise.getType().isPlayer() || (DisguiseConfig.isBedPacketsEnabled() && ((PlayerWatcher) disguise.getWatcher()).isSleeping())))) {
                        packets = new PacketContainer[0];
                    }
                ...
                // Else if the disguise is moving.
                else if (sentPacket.getType() == PacketType.Play.Server.ENTITY_MOVE_LOOK || sentPacket.getType() == PacketType.Play.Server.ENTITY_LOOK || sentPacket.getType() == PacketType.Play.Server.ENTITY_TELEPORT || sentPacket.getType() == PacketType.Play.Server.REL_ENTITY_MOVE) {
                    if (disguise.getType() == DisguiseType.RABBIT && (sentPacket.getType() == PacketType.Play.Server.REL_ENTITY_MOVE || sentPacket.getType() == PacketType.Play.Server.ENTITY_MOVE_LOOK)) {
                        if (entity.getMetadata("LibsRabbitHop").isEmpty() || System.currentTimeMillis() - entity.getMetadata("LibsRabbitHop").get(0).asLong() < 100 || System.currentTimeMillis() - entity.getMetadata("LibsRabbitHop").get(0).asLong() > 500) {
                            if (entity.getMetadata("LibsRabbitHop").isEmpty() || System.currentTimeMillis() - entity.getMetadata("LibsRabbitHop").get(0).asLong() > 500) {
                    ...
                    if (sentPacket.getType() == PacketType.Play.Server.ENTITY_LOOK && disguise.getType() == DisguiseType.WITHER_SKULL) {
                        ...
                        bytes.write(tele ? 1 : 4, getPitch(disguise.getType(), DisguiseType.getType(entity.getType()), pitchValue));
                    ...
                    if (disguise.getType().isPlayer() && entity.getType() != EntityType.PLAYER) {
                        Location loc = entity.getLocation();
                        byte pitch = getPitch(disguise.getType(), DisguiseType.getType(entity.getType()), (byte) (int) (loc.getPitch() * 256.0F / 360.0F));
    ...
    public static PacketContainer[][] constructSpawnPackets(final Player player, Disguise disguise, Entity disguisedEntity) {
        if (disguise.getEntity() == null) { disguise.setEntity(disguisedEntity); }
        ...
        if (disguise.getType() == DisguiseType.EXPERIENCE_ORB) {
            ...
            spawnPackets[0].getDataWatcherModifier().write(0, createDataWatcher(player, WrappedDataWatcher.getEntityWatcher(disguisedEntity), disguise.getWatcher()));
            ...
            if (DisguiseConfig.isBedPacketsEnabled() && ((PlayerWatcher) disguise.getWatcher()).isSleeping()) {
                ...
                PacketContainer[] bedPackets = DisguiseUtilities.getBedPackets(player, loc.clone().subtract(0, PacketsManager.getYModifier(disguisedEntity, disguise), 0), player.getLocation(), ((PlayerDisguise) disguise));
                for (int i = 0; i < 2; i++) {
            ...
            spawnPackets[0] = ProtocolLibrary.getProtocolManager().createPacketConstructor(PacketType.Play.Server.SPAWN_ENTITY, nmsEntity, id, data).createPacket(nmsEntity, id, data);
    ...
    public static double getYModifier(Entity entity, Disguise disguise) {
        ...
        switch (disguise.getType()) {
            case BAT:
                if (entity instanceof LivingEntity) { return yMod + ((LivingEntity) entity).getEyeHeight(); }
            case MINECART:
            case MINECART_COMMAND:
            case MINECART_CHEST:
            case MINECART_FURNACE:
    ...
    public static void init(LibsDisguises plugin) {
        ...
        soundsListener = new PacketAdapter(libsDisguises, ListenerPriority.NORMAL, PacketType.Play.Server.NAMED_SOUND_EFFECT, PacketType.Play.Server.ENTITY_STATUS) {
            @Override
            public void onPacketSending(PacketEvent event) {
                ...
                if (event.getPacketType() == PacketType.Play.Server.NAMED_SOUND_EFFECT) {
                    ...
                    Location soundLoc = new Location(observer.getWorld(), ((Integer) mods.read(1)) / 8D, ((Integer) mods.read(2)) / 8D, ((Integer) mods.read(3)) / 8D);
                            ...
                            if (loc.equals(soundLoc)) {
                                ...
                                if (entitySound != null) {
                                    ...
                                    if (obj == null) {
                                        ...
                                        try {
                                            if (entity instanceof LivingEntity) {
                                                hasInvun = ReflectionManager.getNmsField("Entity", "noDamageTicks").getInt(nmsEntity) == ReflectionManager.getNmsField("EntityLiving", "maxNoDamageTicks").getInt(nmsEntity);
                                            } else {
                                ...
                                if (sound == null) {
                                    ...
                                    if (sound.equals("step.grass")) {
                                        try {
                                            ...
                                            Object block = ReflectionManager.getNmsMethod("RegistryMaterials", "a", int.class).invoke(ReflectionManager.getNmsField("Block", "REGISTRY").get(null), typeId);
                                            if (block != null) {
                            ...
                            if (disSound.getSound(soundType) == null || (disguise.isSelfDisguiseSoundsReplaced() && entity == event.getPlayer())) {
                                ...
                                if (disSound != null) {
                                    ...
                                    if (sound != null) {
                                        ...
                                        if (disguise.getType() == DisguiseType.BAT) { pitch *= 95F; }
                                        pitch *= 63;
                                        if (pitch < 0) { pitch = 0; }
                                        if (pitch > 255) { pitch = 255; }
                                        mods.write(5, (int) pitch);
        ...
        viewDisguisesListener = new PacketAdapter(libsDisguises, ListenerPriority.HIGH, PacketType.Play.Server.NAMED_ENTITY_SPAWN, PacketType.Play.Server.ATTACH_ENTITY, PacketType.Play.Server.REL_ENTITY_MOVE, PacketType.Play.Server.ENTITY_MOVE_LOOK, PacketType.Play.Server.ENTITY_LOOK, PacketType.Play.Server.ENTITY_TELEPORT, PacketType.Play.Server.ENTITY_HEAD_ROTATION, PacketType.Play.Server.ENTITY_METADATA, PacketType.Play.Server.ENTITY_EQUIPMENT, PacketType.Play.Server.ANIMATION, PacketType.Play.Server.BED, PacketType.Play.Server.ENTITY_EFFECT, PacketType.Play.Server.ENTITY_VELOCITY, PacketType.Play.Server.UPDATE_ATTRIBUTES, PacketType.Play.Server.ENTITY_STATUS) {
            @Override
            public void onPacketSending(PacketEvent event) {
                if (event.isCancelled()) { return; }
                final Player observer = event.getPlayer();
                if (event.getPacket().getIntegers().read(0) == observer.getEntityId()) {
                    if (DisguiseAPI.isSelfDisguised(observer)) {
                        ...
                        if (event.getPacketType() == PacketType.Play.Server.ENTITY_METADATA) {
                            ...
                            byte b = (byte) 1 << 5;
                            if (observer.isSprinting()) { b = (byte) (b | 1 << 3); }
                        ...
                        } else if (event.getPacketType() == PacketType.Play.Server.ATTACH_ENTITY || event.getPacketType() == PacketType.Play.Server.REL_ENTITY_MOVE || event.getPacketType() == PacketType.Play.Server.ENTITY_MOVE_LOOK || event.getPacketType() == PacketType.Play.Server.ENTITY_LOOK || event.getPacketType() == PacketType.Play.Server.ENTITY_TELEPORT || event.getPacketType() == PacketType.Play.Server.ENTITY_HEAD_ROTATION || event.getPacketType() == PacketType.Play.Server.ENTITY_EFFECT || event.getPacketType() == PacketType.Play.Server.ENTITY_EQUIPMENT) {
                            event.setCancelled(true);
                        } else if (event.getPacketType() == PacketType.Play.Server.BED) {
                            ReflectionManager.setAllowSleep(observer);
                        } else if (event.getPacketType() == PacketType.Play.Server.ENTITY_STATUS) {
                            Disguise disguise = DisguiseAPI.getDisguise(event.getPlayer(), event.getPlayer());
                            if (disguise.isSelfDisguiseSoundsReplaced() && !disguise.getType().isPlayer() && event.getPacket().getBytes().read(0) == 2) {
                                event.setCancelled(true);
                            }
                        }
                    }
        ...
        inventoryListener = new PacketAdapter(libsDisguises, ListenerPriority.HIGHEST, PacketType.Play.Server.SET_SLOT, PacketType.Play.Server.WINDOW_ITEMS, PacketType.Play.Client.HELD_ITEM_SLOT, PacketType.Play.Client.SET_CREATIVE_SLOT, PacketType.Play.Client.WINDOW_CLICK) {
            @Override
                    ...
                    if (disguise != null && disguise.isSelfDisguiseVisible() && (disguise.isHidingArmorFromSelf() || disguise.isHidingHeldItemFromSelf())) {
                        ...
                        // If the player switched item, aka he moved from slot 1 to slot 2
                            ...
                            if (disguise.isHidingHeldItemFromSelf()) {
                                ...
                                org.bukkit.inventory.ItemStack newHeld = event.getPlayer().getInventory().getItem(event.getPacket().getIntegers().read(0));
            ...
            public void onPacketSending(PacketEvent event) {
                ...
                if (!(event.getPlayer() instanceof com.comphenix.net.sf.cglib.proxy.Factory) && event.getPlayer().getVehicle() == null && event.getPacket().getIntegers().read(0) == 0) {
                    Disguise disguise = DisguiseAPI.getDisguise(event.getPlayer(), event.getPlayer());
                            ...
                            if (slot >= 5 && slot <= 8) {
                                if (disguise.isHidingArmorFromSelf()) {
                                    ...
                                    if (item != null && item.getType() != Material.AIR) {
                                        ...
                                        event.getPacket().getModifier().write(2, ReflectionManager.getNmsItem(new org.bukkit.inventory.ItemStack(0)));
                                    }
                                }
                                // Else if its a hotbar slot
                            } else if (slot >= 36 && slot <= 44) {
